...
- [x] **14단계: 마이페이지 '나의 후원 현황' 서브 탭 레이아웃 교정 및 세부 피그마 명세 동기화**
  - [x] `mypage.html` 내 전체 탭 데이터 뷰 테이블 및 페이징 제거 (요약 박스만 남김)
  - [x] `mypage.html` 내 일시 후원 데이터 테이블 시안 반영 (No, 구분, 캠페인, 결제방법, 금액, 기부일)
  - [x] `button.css` 내 테이블용 높이 36px 전용 버튼(`.btn-table`) 스타일 규칙 추가 정의
  - [x] `login.css` 내 페이징 레이아웃 분할 및 간격 보정 (좌우 화살표 그룹과 숫자 그룹 간 격리 배치)
  - [x] `<` 및 `>` 화살표 버튼 배경 크기 보정(2.4rem) 및 비활성 페이지 버튼 보더 규칙 반영
  - [x] HTML 린트 (`npm run lint:html`) 및 Vite 프로덕션 빌드 검증 완료

- [x] **15단계: 마이페이지 '후원 상세 내역' 화면 구현**
  - [x] `detail.html` 내 후원 상세 정보 요약 표 마크업 구축 및 36px 버튼(`btn-table`) 배치
  - [x] `detail.html` 내 정기 후원 상세 내역 타이틀, 2칸 요약 박스, 테이블(이력 관리 포함) 및 3분할 페이징 구현
  - [x] `login.css` 하단에 후원 상세 정보 요약 표(`.detail-info-table`) 전용 스타일 정의
  - [x] HTML 린트 (`npm run lint:html`) 및 Vite 프로덕션 빌드 검증 완료

- [x] **16단계: 후원 상세 요약 표 이중 분할 및 페이지네이션 간격/배경 피그마 동기화**
  - [x] `detail.html` 내 후원 상세 요약 표를 2개의 독립된 테이블로 분리 마크업
  - [x] `login.css` 내 테이블 셀 높이(8.6rem) 및 패딩(3.0rem 2.4rem), 분할 테이블 간 마진(2rem) 스타일 반영
...
- [x] **18단계: 공지사항 목록 (`notice.html`) 및 상세 (`notice/detail.html`) 구현**
  - [x] `notice.html` 내 정밀 피그마 사양 동기화 마크업 구현
  - [x] `notice/detail.html` 내 정밀 피그마 사양 동기화 마크업 구현
  - [x] `news.css` 내 공지 목록/상세 레이아웃 및 타이포그래피 스타일 신설
  - [x] 데스크탑/모바일 해상도 실측 분기 및 이전/다음 화살표 크기(22x16) 매핑

- [x] **19단계: 활동소식 목록 (`activity.html`) 및 상세 (`activity/detail.html`) 구현**
  - [x] `activity.html` 내 3열 카드 그리드 레이아웃 및 이미지 연동 마크업 구현
  - [x] `activity/detail.html` 내 상세 헤더 및 본문, 첨부파일 영역 마크업 구현
  - [x] `news.css` 내 활동소식 카드 텍스트 배경 라벤더 계열(`#D0C9FF`) 고도화
  - [x] 모바일 뷰포트 그리드 열 흐름(1열 배칭) 및 스타일 분기 최적화

- [x] **20단계: 통합 검증 및 정리**
  - [x] 공통 페이지네이션 연동 검증
  - [x] HTMLHint 정적 린터 검증 (`npm run lint:html`)
